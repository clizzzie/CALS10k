...
c---------------------------------------------------------------------
      ...
      integer lmax,nspl,n,np,nl,jord,nsplt,flag,im,kchosen,tchosen
	...
	real*8 avgdspec(lmax),ssd, ssdd
      ...
      integer it1,it2,lm,nm,k,j,i,nleft,lmm,ntot,ii,m,choice
	...
	data pi/3.14159265359/, a/6371200.d0/, c/3485000.d0/,
     >  sigma/5.0e+05/
      ...
      write(*,*) 'Produces time series of spherical harmonic field'
      write(*,*) 'coefficients and their time average.'
      write(*,*) 'Results are written to 5 plain text output files,'
      write(*,*) 'labeled: FieldCoeffs.txt, SecVarCoeffs.txt,'
      write(*,*) 'Avgs.txt, Spectra.txt, AvgSpectrum.txt. See'
      write(*,*) 'accompanying PDF manual for details.'
c      write(*,*)
...
c
	...
	print *,'1 = Field coefficients versus k, chosen t'
	print *,'2 = Field coefficients versus t, chosen k'
	print *,'3 = Secular variation coefficients versus k, chosen t'
	print *,'4 = Secular variation coefficients versus t, chosen k'
	print *,'5 = Time-avg. field coefficients versus k'
	print *,'6 = Time-avg. sec. var. coeff. vs k'
	print *,'7 = Time-standard dev. field coeff. vs k'
	print *,'8 = Time-standard dev. sec. var. coeff. vs k'
	print *,'9 = Spectral estimate of field coeff. vs t, chosen k'
	print *,'10 = Spectral est. of field coeff. vs k, chosen t'
	print *,'11 = Spectral est. of sec. var. coeff. vs t, chosen k'
	print *,'12 = Spectral est. of sec. var. coeff. vs k, chosen t'
	print *,'13 = Time-avg. spectral est. of field coeff. vs k'
	print *,'14 = Time-avg. spectral est. of sec. var. coeff. vs k'
	read(5,*)choice
	...
	print *,'Which version of CALS10k would you like to use? 1= .1b,2=.OL2, 3=.OL2c,
     $  4=.OL2w, 5=.1, 6=.OL2hd, 7=.OL2ld, 8=.2, 9=ARCH10k.0,
     $  10=ARCH10k.0_40'
	read(5,*)im
	if (im.eq.1) then
      open(1,file='CALS10k.1b
     $')
	print *,' Using CALS10k.1b'
	else if (im.eq.2) then
      open(1,file='CALS10k.OL2
     $')
...
c      write(*,*) 'Give output file name for coeffs:'
c      read(*,*) outfile
c      write(*,*) 'Give output file name for svcoeffs:'
c      read(*,*) svoutfile
c      write(*,*) 'Give output file name for average:'
c      read(*,*) avfile
c      write(*,*) 'Give output file name for spectra:'
c      read(*,*) specfile
c      write(*,*) 'Give output file name for average of spectra:'
c      read(*,*) avspecfile
      write(*,*) 'Give  max desired degree l'
      read(*,*) lmm
	ntot=lmm*(lmm+2)

      print  '(a)', ' Enter spectral evaluation radius btwn.
     > c=.54701 and a=1 '
	read (5,*) radius
	rsq=radius**2
c
	...
	print *, 'interval  ', it1,it2
      open(11,file='FieldCoeffs.txt')
      open(12,file='Avgs.txt')
      open(13,file='Spectra.txt')
      open(14,file='AvgSpectrum.txt')
      open(15,file='SecVarCoeffs.txt')
...
6200  format(i6,24(1x,e12.6))
      ...
      write(11,*) lmm,radius,real(it1),real(it2)
      write(12,*) lmm,radius,real(it1),real(it2)
      write(13,*) lmm,radius,real(it1),real(it2)
      write(14,*) lmm,radius,real(it1),real(it2)
      write(15,*) lmm,radius,real(it1),real(it2)
	...
	if (choice.eq.1) then
	  ...
	  write(13,*) kchosen,1,9
	else if (choice.eq.10) then
	  write(13,*) tchosen,0,10
	else if (choice.eq.11) then
	  write(13,*) kchosen,1,11
	else if (choice.eq.12) then
	  write(13,*) tchosen,0,12
...
c*******Note the option originally picked by the user, for later processing.************
      write(14,*) choice
